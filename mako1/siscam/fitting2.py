...
"""
    ...
    print ('The values of the peaks found by the computer are...')
...
class Fitting(object):
    ...
    def do_fit(self, img, img1, img2, roi):
        """perform fitting.
...
class Gauss2d(Fitting):
    ...
    def do_fit(self, img, img1, img2, roi):
        ...
        rel = numpy.sum(img2)/numpy.sum(img1)
...
class FluorescenceGauss(Fitting):
    ...
    def _find_startpar_gauss(self, x, prof):
        ...
        #estimate peak position and fwhm width
        peakval, peakpos = pickpeak(profsmooth, 3)
        ...
        try:
            ...
            halfval2, halfpos2 = pickpeak(
                ...
                npicks = 3)
            width1 = 15 #numpy.abs(numpy.diff(xs[halfpos1]))
            width2 = 15 #numpy.abs(numpy.diff(xs[halfpos2]))
        ...
        off = numpy.nanmin(profsmooth) #TODO: can we do better (robust?)
        print ('Hello its me: the value of the offset is::::: tatatachan')        
        print (off)
        try:
            m1 = xs[peakpos[0]]
            m2 = xs[peakpos[1]]
    ...
    def do_fit(self, img, img1, img2, roi):
        ...
        try:
            ...
            startparx = numpy.array([1,1, 80, 120, 10, 10, 0])
            startpary = numpy.array([1,1, 80, 120, 10, 10, 0])
            print "Warning: can't determine initial guess for fitting parameters"
...
class FitParsGauss2d(FitPars):
    ...
    def N(self):
        ...
        N = 1e-3 * 2*numpy.pi * \
            (self.A/45.) * self.sx*1e-6 * self.sy*1e-6 / self.imaging_pars.sigma0 #this is for absorption, the 45 comes from a factor in Acquire for contrast (line 364)
...
class FitParsFluorescenceGauss(FitPars):
    ...
    def N(self):
        "atom number in thousand"
        N = 1e-3 * 2*numpy.pi * \
            (self.A1/45.) * self.sx*1e-6 * self.sy*1e-6 / self.imaging_pars.sigma0
        return N
    @property
    def Nerr(self):
        "error atom number"
    ...
    def N2(self):
        "atom number in thousand"
        N2 = 1e-3 * 2*numpy.pi * \
            (self.A2/45.) * self.sx2*1e-6 * self.sy2*1e-6 / self.imaging_pars.sigma0
        return N2
...
class FitParsBimodal2d(FitParsGauss2d):
    ...
    def Nth(self):
        "number of atoms in thermal cloud"
        Nth = 1e-3 * 2*numpy.pi * \
              self.A/45. * self.sx*1e-6 * self.sy*1e-6 / self.imaging_pars.sigma0 
        return Nth
    ...
    def Nbec(self):
        ...
        Nbec = 1e-3*1.25* \
               self.B/45. * self.rx*1e-6 * self.ry*1e-6 / self.imaging_pars.sigma0 
        return Nbec

    @property
    def OD(self):
        "optical density"
        return (self.A + self.B)/45.0  # from the scaling factor in acquire
...
class FitParsTF2d(FitPars):
    ...
    def OD(self):
        "optical density"
        return self.B/45.0   # scaling factor, the same as in gauss 2d fit 
...
def test_fitting_bimodal():
    ...
    imgfit, background, fitpars = fit2.do_fit(numpy.ma.array(img), numpy.ma.array(img1), numpy.ma.array(img2), roi.ROI( 100, 400, 10, 190))
    imgfitg, background, fitparsg = fitg.do_fit(numpy.ma.array(img), numpy.ma.array(img1), numpy.ma.array(img2), roi.ROI( 100, 400, 10, 190))
    imgfitth, background, fitparstf = fittf.do_fit(numpy.ma.array(img), numpy.ma.array(img1), numpy.ma.array(img2), roi.ROI( 100, 400, 10, 190))
...
def test_fitting_bimodal_startpars():
    ...
    imgfit, background, fitpars = fit.do_fit(numpy.ma.array(img), numpy.ma.array(img1), numpy.ma.array(img2), r)
...
def test_fitting_gaussbose():
    ...
    imgfit, background, fitpars = fit.do_fit(numpy.ma.array(img), numpy.ma.array(img1), numpy.ma.array(img2), roi.ROI( 100, 400, 10, 190))
