...
class Bimodal2d(Gauss2d):
    ...
    def Dbimodal2d(self, pars, x, y, v=0):
        ...
        J[1] = 1.0/sx**2*g*(x-mx)+ 1.5*B*b*(2.0*numpy.cos(theta)*(x*numpy.cos(theta)+y*numpy.sin(theta)-mx*numpy.cos(theta)-my*numpy.sin(theta))/(rx**2)-2.0*numpy.sin(theta)*(-x*numpy.sin(theta)+y*numpy.cos(theta)+mx*numpy.sin(theta)-my*numpy.cos(theta))/(ry)**2)
        J[2] = 1.0/sx**2*g*(y-my)+ 1.5*B*b*(2.0*numpy.sin(theta)*(x*numpy.cos(theta)+y*numpy.sin(theta)-mx*numpy.cos(theta)-my*numpy.sin(theta))/(rx**2)+2.0*numpy.cos(theta)*(-x*numpy.sin(theta)+y*numpy.cos(theta)+mx*numpy.sin(theta)-my*numpy.cos(theta))/(ry)**2)
        J[3] = (1.0/sx**3) * g * ((x-mx)**2)
        J[4] = (1.0/sx**3) * g * ((y-my)**2)
        J[5] = 1
        J[6] = b**3
        J[7] = (3.0*B/(rx**3)) * (x*numpy.cos(theta)+y*numpy.sin(theta)-mx*numpy.cos(theta)-my*numpy.sin(theta))**2  * b
        J[8] = (3.0*B/(ry**3)) * (-x*numpy.sin(theta)+y*numpy.cos(theta)+mx*numpy.sin(theta)-my*numpy.cos(theta))**2 * b
        J[9] = -3.0*B*b*((((x*numpy.cos(theta)+y*numpy.sin(theta)-mx*numpy.cos(theta)-my*numpy.sin(theta)))*(-x*numpy.sin(theta)+y*numpy.cos(theta)+mx*numpy.sin(theta)-my*numpy.cos(theta)))/rx**2-(((x*numpy.cos(theta)+y*numpy.sin(theta)-mx*numpy.cos(theta)-my*numpy.sin(theta)))*(-x*numpy.sin(theta)+y*numpy.cos(theta)+mx*numpy.sin(theta)-my*numpy.cos(theta)))/ry**2)        
    ...
    def rF(self, pars, x, y):
        ...
        numpy.multiply(F[1], 1.5*(2.0*numpy.cos(theta)*(x*numpy.cos(theta)+y*numpy.sin(theta)-mx*numpy.cos(theta)-my*numpy.sin(theta))/(rx**2)-2.0*numpy.sin(theta)*(-x*numpy.sin(theta)+y*numpy.cos(theta)+mx*numpy.sin(theta)-my*numpy.cos(theta))/(ry)**2), Fdmx[1])
        Fdmx[2] = 0

        #my
        Fdmy = numpy.empty_like(F)
        numpy.multiply(F[0], 1.0/(sx**2)*(y-my), Fdmy[0])
        numpy.multiply(F[1], 1.5*b*(2.0*numpy.sin(theta)*(x*numpy.cos(theta)+y*numpy.sin(theta)-mx*numpy.cos(theta)-my*numpy.sin(theta))/(rx**2)+2.0*numpy.cos(theta)*(-x*numpy.sin(theta)+y*numpy.cos(theta)+mx*numpy.sin(theta)-my*numpy.cos(theta))/(ry)**2), Fdmy[1])
        Fdmy[2] = 0
...
class FitParsFluorescenceGauss(FitPars):
    ...
    def N(self):
        "atom number in thousand"
        N = 1e-3 * 2*numpy.pi * \
            (self.A1/100.) * self.sx*1e-6 * self.sy*1e-6 / self.imaging_pars.sigma0
        return N
    @property
    def Nerr(self):
        "error atom number"
...
class FitParsTF2d(FitPars):
    ...
    def OD(self):
        "optical density"
        return self.B/5.0   # scaling factor, the same as in gauss 2d fit 
