    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                img =  -((np.log(np.abs(img1 - img3)) - np.log(np.abs(img2 - img3))))
    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                self.save_abs_img(settings.testfile, img)
                self.save_abs_img(settings.absorbfile, img1)
                self.save_abs_img(settings.forefile, img2)
                self.save_abs_img(settings.backfile, img3)
    ...
    def run(self):
        ...
        while self.running:
            if not self.queue.empty():
                ...
                #calculate absorption image
                img = img1.astype(np.int16)-img2.astype(np.int16)####changed np.int16 to np.uint16
...
class ImgAcquireApp(wx.App):
    ...
    def OnInit(self):
        ...
        #self.timing_exposure_theta = 100 #�s
        #self.timing_repetition_theta = 600 #ms
        #self.timing_exposure_bluefox = 20000 #�s
...
##### ----------- New AVT - Section ---------------- Added on 19022015
    ...
    def OnDoubleImageAcquiredAVT(self,event):
        img1 = event.data['image1']
        #print "img 1 acquired!", time.time()
        img2 = event.data['image2']
        #print "img 2 acquired!", time.time()
...
class TimingDialog(wx.Dialog):
    def __init__(self, parent, ID, title,
        ...
        #TODO: enable float entry
        box = wx.BoxSizer(wx.HORIZONTAL)
        label = wx.StaticText(self, - 1, "Exposure time (�s)")
        box.Add(label, 0, wx.ALIGN_CENTRE | wx.ALL, 5)
        entry = wx.SpinCtrl(self, - 1, "", (50, - 1))
        #entry = wx.SpinButton(self, -1, style = wx.SP_VERTICAL)
        entry.SetRange(0, 1000000)
