...
class ImgPanel(wx.Panel):
    ...
    def __init__(self, parent, panel_id, img, region_of_interest, fitroutine):
        ...
        self.himg = self.aximg.imshow(img, vmin= - 0.05, vmax=128,
                                      origin="upper",
        ...
        try:
            ...
            if self.show_contours:
                ...
                if isinstance(self.fitpars, fitting.FitParsBimodal2d):
                    ...
                    imgfitgauss = (self.imgfit[0] - self.img_background)
                    cs = self.aximg.contour(self.roi.xrange_clipped(self.img),
                               self.roi.yrange_clipped(self.img),
                               #self.imgfit[0],
                               imgfitgauss,                                 
                    ...
                    self.hcontours = cs.collections
                #(jsc)    
                elif isinstance(self.fitpars, fitting.FitParsFluorescenceGauss):
                    imgfitgauss = (self.imgfit[0] - self.img_background)
                    cs = self.aximg.contour(self.roi.xrange_clipped(self.img),
                               self.roi.yrange_clipped(self.img),
                               #self.imgfit[0],
    ...
    def update(self):
        "calculate profiles and fit results, peorm redraw"
...
class ImgAppAui(wx.App):
    ...
    ID_FitRbFluorescenceGauss = wx.NewId()
    ...
    ID_FitKTF      = wx.NewId()
    ID_FitKFluorescenceGauss = wx.NewId()

    ID_FitSrNone    = wx.NewId()
    ID_FitSrGauss   = wx.NewId()
    ID_FitSrBimodal = wx.NewId()
    ID_FitSrTF      = wx.NewId()
    ID_FitSrFluorescenceGauss = wx.NewId()
    
    
    ID_CompensateOffsetRb = wx.NewId()
    ID_CompensateOffsetK = wx.NewId()
    ID_CompensateOffsetSr = wx.NewId()
    ...
    #other settings
    imagefilename = settings.testfile  #settings.imagefile
    imagefilename1 = settings.absorbfile
    imagefilename2 = settings.forefile
    imagefilename3 = settings.backfile
    ...
    roiRb = [ROI(0, 640, 0, 480), ROI(0, 640, 0, 480), ROI(0, 640, 0, 480)]# Changed to matchg GUPPY 031B camera size. Initial ROI is full img
    roiK = [ROI(0, 640, 0, 480), ROI(0, 640, 0, 480), ROI(0, 640, 0, 480)]
    roiSr = [ROI(0, 640, 0, 480), ROI(0, 640, 0, 480), ROI(0, 640, 0, 480)]
    ...
    for i in range(len(imaging_parlist)):
        imaging_parlist[i]['Rb'].mass = 87.0 * 1.660539040e-27
...
#    97
    ...
    def select_roi(self, sel):
        ...
        self.Sr.set_roi(self.roiSr[sel])
...
#------------------------------------------------------------------------------           
        ...
        if id in [self.ID_FitRbBimodal]:
            ...
            self.Rb.update()
        #(jsc)
        if id in [self.ID_FitRbFluorescenceGauss]:
            self.Rb.fit = fitting.FluorescenceGauss(self.imaging_pars['Rb'])
            self.Rb.update()
            
#------------------------------------------------------------------------------
        if id in [self.ID_FitKNone]:
            self.K.fit = fitting.NoFit(self.imaging_pars['Rb'])
            self.K.update()
            
        if id in [self.ID_FitKGauss]:
            self.K.fit  = fitting.Gauss2d(self.imaging_pars['Rb'])
            self.K.update()
            
        if id in [self.ID_FitKBimodal]:   
            self.K.fit  = fitting.Bimodal2d(self.imaging_pars['Rb'])
            self.K.update()
            
        if id in [self.ID_FitKTF]:
            self.K.fit = fitting.ThomasFermi2d(self.imaging_pars['Rb'])
            self.K.update()    
        #(jsc)
        if id in [self.ID_FitKFluorescenceGauss]:
            self.K.fit = fitting.FluorescenceGauss(self.imaging_pars['Rb'])
            self.K.update()  

#------------------------------------------------------------------------------   
        if id in [self.ID_FitSrNone]:
            self.Sr.fit = fitting.NoFit(self.imaging_pars['Rb'])
            self.Sr.update()
            
        if id in [self.ID_FitSrGauss]:    
            self.Sr.fit = fitting.Gauss2d(self.imaging_pars['Rb'])
            self.Sr.update()            
        
        if id in [self.ID_FitSrTF]:    
            self.Sr.fit = fitting.ThomasFermi2d(self.imaging_pars['Rb'])
            self.Sr.update()
            
        if id in [self.ID_FitSrBimodal]:
            self.Sr.fit = fitting.Bimodal2d(self.imaging_pars['Rb'])
            self.Sr.update()
        #(jsc)    
        if id in [self.ID_FitSrFluorescenceGauss]:
            self.Sr.fit = fitting.FluorescenceGauss(self.imaging_pars['Rb'])
            self.Sr.update()
#------------------------------------------------------------------------------            
    ...
    def OnMenuCompensate(self, event):
        ...
        if id in [self.ID_CompensateOffsetSr]:
            state = self.menu.FindItemById(id).IsChecked()
            self.Sr.set_compensate_offset(True)
        else:
            self.Sr.set_compensate_offset(True)
    ...
    def OnMenuContrast(self, event):
        ...
        Rb_contrast_dict = {
            ...
            self.ID_Rb_ContrastLow:    20,#inverted, see img panel
            }
        K_contrast_dict = Rb_contrast_dict
        Sr_contrast_dict = Rb_contrast_dict

        cmax = Rb_contrast_dict.get(id)
        cmax = K_contrast_dict.get(id)
        cmax = Sr_contrast_dict.get(id)
        ...
        if cmax:
            ...
            self.K.redraw()
            self.Sr.himg.set_clim(vmax=cmax)
            self.Sr.redraw()
    ...
    def OnSaveImageEvent(self, event):
        ...
        #(jsc) saving only the results 
        '''
        imagesavefilename1 = "abs_%s_%04d.png" % (self.results.name,
                                             self.results.active_row)
        imagesavefilenamefull1 = os.path.normpath(os.path.join(imagesavedir, imagesavefilename1))
        
        imagesavefilename2 = "fore_%s_%04d.png" % (self.results.name,
                                             self.results.active_row)
        imagesavefilenamefull2 = os.path.normpath(os.path.join(imagesavedir, imagesavefilename2))
        
        imagesavefilename3 = "back_%s_%04d.png" % (self.results.name,
                                             self.results.active_row)
        imagesavefilenamefull3 = os.path.normpath(os.path.join(imagesavedir, imagesavefilename3))
        '''
        ...
        shutil.copy2(self.imagefilename1,imagesavefilenamefull1)
        shutil.copy2(self.imagefilename2,imagesavefilenamefull2)
        shutil.copy2(self.imagefilename3,imagesavefilenamefull3)
 ...
 | wx.aui.AUI_MGR_ALLOW_FLOATING
        ...
        fit_menu.AppendRadioItem(
            self.ID_FitRbFluorescenceGauss,
            "Rb FLuorescence Gauss fit",
...
#------------------------------------------------------------------------------    
        ...
        fit_menu.AppendRadioItem(
            self.ID_FitKNone,
            "K no fit",
            "perform no fit for Calcium",
            )  
            
        fit_menu.AppendRadioItem(
            self.ID_FitKGauss,
            "K Gauss fit",
            "perform 2d Gauss fit for Calcium",
            )
            
        fit_menu.AppendRadioItem(
            self.ID_FitKTF,
            "K Thomas-Fermi fit",
            "perform 2d Thomas-Fermi fit distribution for Calcium",
            )
            
        fit_menu.AppendRadioItem(
            self.ID_FitKBimodal,
            "K bimodal fit",
            "perform 2d fit of Gaussian + Thomas-Fermi distribution for Calcium",
            )
        #(jsc)
        fit_menu.AppendRadioItem(
            self.ID_FitKFluorescenceGauss,
            "K FLuorescence Gauss fit",
            "perform 2d Gauss fit for Calcium",
            )            
...
#------------------------------------------------------------------------------
        ...
        fit_menu.AppendRadioItem(
            self.ID_FitSrNone,
            "Sr no fit",
            "perform no fit for Strontium",
            )              
            
        fit_menu.AppendRadioItem(
            self.ID_FitSrGauss,
            "Sr Gauss fit",
            "perform 2d Gauss fit for Strontium",
            )   
            
        fit_menu.AppendRadioItem(
            self.ID_FitSrBimodal,
            "Sr bimodal fit",
            "perform 2d fit of Gaussian + Thomas-Fermi distribution for Strontium",
            )
        
        fit_menu.AppendRadioItem(
            self.ID_FitSrTF,
            "Sr Thomas-Fermi fit",
            "perform 2d Thomas-Fermi fit distribution for Strontium",
            )
            
        #(jsc)    
        fit_menu.AppendRadioItem(
            self.ID_FitSrFluorescenceGauss,
            "Sr FLuorescence Gauss fit",
            "perform 2d Gauss fit for Strontium",
            )
...
#------------------------------------------------------------------------------            
        ...
        fit_menu.Append(self.ID_CompensateOffsetK,
                        ...
                        'Compensate offset of image background',
                        wx.ITEM_CHECK)
        fit_menu.Append(self.ID_CompensateOffsetSr,
                        'Sr compensate background',
                        'Compensate offset of image background',
                        wx.ITEM_CHECK)
        ...
        self.frame.Bind(wx.EVT_MENU_RANGE,
                        self.OnMenuCompensate,
                        id=self.ID_CompensateOffsetK)
        self.frame.Bind(wx.EVT_MENU_RANGE,
                        self.OnMenuCompensate,
                        id=self.ID_CompensateOffsetSr)
                        
        ##adding copies of each type of function
        self.frame.Bind(wx.EVT_MENU_RANGE,
                        self.OnMenuFit,
                        id=self.ID_FitRbGauss)
        self.frame.Bind(wx.EVT_MENU_RANGE,
                        self.OnMenuFit,
                        id=self.ID_FitRbGaussSym)
        self.frame.Bind(wx.EVT_MENU_RANGE,
                        self.OnMenuFit,
                        id=self.ID_FitRbFluorescenceGauss)
        self.frame.Bind(wx.EVT_MENU_RANGE,
                        self.OnMenuFit,
                        id=self.ID_FitRbGaussBose)
        self.frame.Bind(wx.EVT_MENU_RANGE,
                        self.OnMenuFit,
                        id=self.ID_FitRbBimodal)
                        
        self.frame.Bind(wx.EVT_MENU_RANGE,
                        self.OnMenuFit,
                        id=self.ID_FitSrBimodal)
        self.frame.Bind(wx.EVT_MENU_RANGE,
                        self.OnMenuFit,
                        id=self.ID_FitKBimodal)
        self.frame.Bind(wx.EVT_MENU_RANGE,
                        self.OnMenuFit,
                        id=self.ID_FitRbBoseBimodal)
        self.frame.Bind(wx.EVT_MENU_RANGE,
                        self.OnMenuFit,
                        id=self.ID_FitRbTF)
        ...
        self.frame.Bind(wx.EVT_MENU_RANGE,
                        self.OnMenuFit,
                        id=self.ID_FitSrGauss)
        self.frame.Bind(wx.EVT_MENU_RANGE,
                        self.OnMenuFit,
                        id=self.ID_FitKGauss)
                        
        #(jsc)                
        self.frame.Bind(wx.EVT_MENU_RANGE,
                        self.OnMenuFit,
                        id=self.ID_FitKFluorescenceGauss) 
        self.frame.Bind(wx.EVT_MENU_RANGE,
                        self.OnMenuFit,
                        id=self.ID_FitSrFluorescenceGauss) 
        #(jsc)               
        self.frame.Bind(wx.EVT_MENU_RANGE,
                        self.OnMenuFit,
                        id=self.ID_FitKNone)   
        ...
        imgSr, imgK, imgRb = loadimg(self.imagefilename),loadimg(self.imagefilename),loadimg(self.imagefilename)
        imgSr = ma.array(imgSr, mask= ~ numpy.isfinite(imgSr))        
        imgK = ma.array(imgK, mask= ~ numpy.isfinite(imgK))
        imgRb = ma.array(imgRb, mask= ~ numpy.isfinite(imgRb))
                
        self.Sr = ImgPanel(self.frame, 'Sr', imgSr, self.roiSr[0], fitting.Gauss2d(self.imaging_pars['Rb']))
        self.K = ImgPanel(self.frame, 'K', imgK, self.roiK[0], fitting.Gauss2d(self.imaging_pars['Rb']))
        ...
        #(jsc) adding 3 new toolbars #########################################
    
        self.tb3 = wx.ToolBar(self.frame, - 1, wx.DefaultPosition,
                             wx.DefaultSize,
                             wx.TB_FLAT | wx.TB_NODIVIDER
                             )
        self.tb3.SetToolBitmapSize((24, 24)) 

        self.tb4 = wx.ToolBar(self.frame, - 1, wx.DefaultPosition,
                             wx.DefaultSize,
                             wx.TB_FLAT | wx.TB_NODIVIDER
                             )
        self.tb4.SetToolBitmapSize((24, 24)) 

        self.tb5 = wx.ToolBar(self.frame, - 1, wx.DefaultPosition,
                             wx.DefaultSize,
                             wx.TB_FLAT | wx.TB_NODIVIDER
                             )
        self.tb5.SetToolBitmapSize((24, 24)) 
#------------------------------------------------------------------------------      
        
        self.n1button  = wx.Button(self.tb3, self.ID_FitSrNone, 
                                  "No fit", size = (100,30) ) 
        self.tb3.AddControl(self.n1button)
        self.frame.Bind(wx.EVT_BUTTON, self.OnMenuFit, id=self.ID_FitSrNone)        
        
        
        self.g1button  = wx.Button(self.tb3, self.ID_FitSrGauss, 
                                  "Symmetric Gauss fit", size = (100,30) ) 
        self.tb3.AddControl(self.g1button)
        self.frame.Bind(wx.EVT_BUTTON, self.OnMenuFit, id=self.ID_FitSrGauss)
        
        
        self.fg1button  = wx.Button(self.tb3, self.ID_FitSrFluorescenceGauss, 
                                  "Gauss fit", size = (100,30) ) 
        self.tb3.AddControl(self.fg1button)
        self.frame.Bind(wx.EVT_BUTTON, self.OnMenuFit, id=self.ID_FitSrFluorescenceGauss)        
                      
        self.bm1button = wx.Button(self.tb3, self.ID_FitSrBimodal, 
                                  "Bimodal fit", size = (100,30) )
        self.tb3.AddControl(self.bm1button)
        self.frame.Bind(wx.EVT_BUTTON, self.OnMenuFit, id=self.ID_FitSrBimodal)   
        
        self.tf1button = wx.Button(self.tb3, self.ID_FitSrTF, 
                                   "Thomas-Fermi fit", size = (100,30) )
        self.tb3.AddControl(self.tf1button)
        self.frame.Bind(wx.EVT_BUTTON, self.OnMenuFit, id=self.ID_FitSrTF)  
                                  
#------------------------------------------------------------------------------  
        
        self.n2button  = wx.Button(self.tb4, self.ID_FitKNone, 
                                  "No fit", size = (100,30) ) 
        self.tb4.AddControl(self.n2button)
        self.frame.Bind(wx.EVT_BUTTON, self.OnMenuFit, id=self.ID_FitKNone)        
        
        
        self.g2button  = wx.Button(self.tb4, self.ID_FitKGauss, 
                                  "Symmetric Gauss fit", size = (100,30) ) 
        self.tb4.AddControl(self.g2button)
        self.frame.Bind(wx.EVT_BUTTON, self.OnMenuFit, id=self.ID_FitKGauss)
        
        
        self.fg2button  = wx.Button(self.tb4, self.ID_FitKFluorescenceGauss, 
                                  "Gauss fit", size = (100,30) ) 
        self.tb4.AddControl(self.fg2button)
        self.frame.Bind(wx.EVT_BUTTON, self.OnMenuFit, id=self.ID_FitKFluorescenceGauss)          
                                 
        self.bm2button = wx.Button(self.tb4, self.ID_FitKBimodal, 
                                  "Bimodal fit", size = (100,30) )
        self.tb4.AddControl(self.bm2button)
        self.frame.Bind(wx.EVT_BUTTON, self.OnMenuFit, id=self.ID_FitKBimodal)                                  
                                  
        self.tf2button = wx.Button(self.tb4, self.ID_FitKTF, 
                                   "Thomas-Fermi fit", size = (100,30) )
        self.tb4.AddControl(self.tf2button)
        self.frame.Bind(wx.EVT_BUTTON, self.OnMenuFit, id=self.ID_FitKTF)
                                   
#------------------------------------------------------------------------------
                                   
        self.n3button  = wx.Button(self.tb5, self.ID_FitRbNone, 
                                  "No fit", size = (100,30) ) 
        self.tb5.AddControl(self.n3button)
        self.frame.Bind(wx.EVT_BUTTON, self.OnMenuFit, id=self.ID_FitRbNone)   
                                   
        
        self.g3button  = wx.Button(self.tb5, self.ID_FitRbGauss, 
                                  "Symmetric Gauss fit", size = (100,30) ) 
        self.tb5.AddControl(self.g3button)
        self.frame.Bind(wx.EVT_BUTTON, self.OnMenuFit, id=self.ID_FitRbGauss)
        

        self.fg3button  = wx.Button(self.tb5, self.ID_FitRbFluorescenceGauss, 
                                  "Gauss fit", size = (100,30) ) 
        self.tb5.AddControl(self.fg3button)
        self.frame.Bind(wx.EVT_BUTTON, self.OnMenuFit, id=self.ID_FitRbFluorescenceGauss)           
                                  
        self.bm3button = wx.Button(self.tb5, self.ID_FitRbBimodal, 
                                  "Bimodal fit", size = (100,30) )
        self.tb5.AddControl(self.bm3button)
        self.frame.Bind(wx.EVT_BUTTON, self.OnMenuFit, id=self.ID_FitRbBimodal)     
                             
        self.tf3button = wx.Button(self.tb5, self.ID_FitRbTF, 
                                   "Thomas-Fermi fit", size = (100,30) )
        self.tb5.AddControl(self.tf3button)
        self.frame.Bind(wx.EVT_BUTTON, self.OnMenuFit, id=self.ID_FitRbTF)                                   
        ...
        #(jsc) adding 3 new tool bars for the fittings. 
        self.tb3.Realize()
        self.tb4.Realize() 
        self.tb5.Realize()
    ...
    def load_image(self, filename, target = None):
        ...
        imgSr,imgK,imgRb = loadimg(filename),loadimg(filename),loadimg(filename)
        ...
        self.Sr.analyze_image(imgSr, target)        
