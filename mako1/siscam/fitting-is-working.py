...
class FitParsGauss2d(FitPars):
    ...
    def N(self):
        ...
        N = 1e-3 * 2*numpy.pi * \
            (self.A/5.) * self.sx*1e-6 * self.sy*1e-6 / self.imaging_pars.sigma0 #this is for absorption, the 45 comes from a factor in Acquire for contrast (line 364)
...
class FitParsBimodal2d(FitParsGauss2d):
    ...
    def Nth(self):
        "number of atoms in thermal cloud"
        Nth = 1e-3 * 2*numpy.pi * \
              self.A/5. * self.sx*1e-6 * self.sy*1e-6 / self.imaging_pars.sigma0 
        return Nth
    ...
    def Nbec(self):
        ...
        Nbec = 1e-3*1.25* \
               self.B/5. * self.rx*1e-6 * self.ry*1e-6 / self.imaging_pars.sigma0 
        return Nbec

    @property
    def OD(self):
        "optical density"
        return (self.A + self.B)/5.0  # from the scaling factor in acquire
